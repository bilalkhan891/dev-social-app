...
const { profileValidation, validationResult } = require("../../validations/profile-validation.js")
const { experienceValidation } = require("../../validations/experience-validation.js")
const { educationValidation } = require("../../validations/education-validation.js")
...
router.put("/experience", [experienceValidation, auth], async (req, res) => {
  ...
  try {
    ...
    const experience = await profile.save()

    return res.json({ msg: "experience saved!", experience })
...
router.delete("/experience/:experience_id", auth, async (req, res) => {
  try {
    ...
    const profile = await Profile.findOneAndUpdate({ user: req.user.id }, { $pull: { experience: { _id: req.params.education_id } } })
    console.log(profile)
...
router.put("/education", [educationValidation, auth], async (req, res) => {
  ...
  const newEdu = { school, degree, fieldofstudy, location, from, to, current, description }
  try {
    // Find user in user collection
    const profile = await Profile.findOne({ user: req.user.id })
    profile.education.unshift(newEdu)
    const education = await profile.save()

    return res.json({ msg: "education saved!", education })
...
router.delete("/education/:education_id", auth, async (req, res) => {
  try {
    // Find user in user collection
    console.log(req.params)
    const profile = await Profile.findOneAndUpdate({ user: req.user.id }, { $pull: { education: { _id: req.params.education_id } } })
    console.log(profile)
